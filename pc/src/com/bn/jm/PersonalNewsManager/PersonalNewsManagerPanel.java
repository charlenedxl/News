...
public class PersonalNewsManagerPanel extends JPanel {
	// ======================================个人新闻列表======================================
	// 个人新闻每一列的类型
	Class[] typeArray = { Integer.class, String.class, String.class, String.class, String.class, String.class,
	...
	JTable jtPersonalNews = new JTable() {
		@Override
	...
	// 个人新闻表格
	PersonalNewsTableModel tmPersonalNews;

	// 存放表格的JScrollPane
	JScrollPane jspPersonalNews = new JScrollPane(jtPersonalNews);
	...
	public PersonalNewsManagerPanel(MainJFrame mf, String ygid) {
		this.ygid = ygid;
		this.mf = mf;
		this.setLayout(null);
		...
		// 表头设置
		jspPersonalNews.setBounds(25, 50, 1135, 470);
		jlTitle.setBounds(520, 20, 200, 20);
		jlTitle.setFont(subtitle);

		this.add(jlTitle);
		this.add(jspPersonalNews);
		this.add(jbCKSH);
		initFilterPane();
		addFilterListeners();
		addTableListeners();
	...
	public void initTable() {
		// 设置表头绘制器
		jtPersonalNews.getTableHeader().setUI(new GroupableTableHeaderUI());
		// 设置行高
		jtPersonalNews.setRowHeight(30);
		// 设置只能单选
		jtPersonalNews.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		// 获得table单元格绘制器
		DefaultTableCellRenderer dtcr = new DefaultTableCellRenderer();
		// 设置表格里内容居中
		dtcr.setHorizontalAlignment(SwingConstants.CENTER);
		// 设置第一列居中
		jtPersonalNews.setDefaultRenderer(Integer.class, dtcr);

		// 为修改按钮和删除按钮添加绘制器
		ButtonRenderer jButtonRenderer = new ButtonRenderer();
		jtPersonalNews.setDefaultRenderer(JButton.class, jButtonRenderer);

		// 获得表头
		JTableHeader tableHeader = jtPersonalNews.getTableHeader();
		// 获得表头绘制器
		...
		// 获得每一列的引用
		TableColumn tc0 = jtPersonalNews.getColumnModel().getColumn(0);
		TableColumn tc1 = jtPersonalNews.getColumnModel().getColumn(1);
		TableColumn tc2 = jtPersonalNews.getColumnModel().getColumn(2);
		TableColumn tc3 = jtPersonalNews.getColumnModel().getColumn(3);
		TableColumn tc4 = jtPersonalNews.getColumnModel().getColumn(4);
		TableColumn tc5 = jtPersonalNews.getColumnModel().getColumn(5);
		TableColumn tc6 = jtPersonalNews.getColumnModel().getColumn(6);
		TableColumn tc7 = jtPersonalNews.getColumnModel().getColumn(7);
		...
		UpdateButtonEditor updateButtonEidtor = new UpdateButtonEditor(this, mf);
		tc6.setCellEditor(updateButtonEidtor);
		// 为删除按钮添加编辑器
		tc7.setPreferredWidth(120);
		DeleteButtonEditor deleteButtonEidtor = new DeleteButtonEditor(this);
		tc7.setCellEditor(deleteButtonEidtor);
	...
	public void addTableListeners() {// 添加表格的监听器
		jtPersonalNews.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent arg0) {
				final int row = jtPersonalNews.getSelectedRow();
				if (pressedRow != row) {// 如果点击的不在同一行
					pressedRow = row;
				}
			}
		...
		jtPersonalNews.addKeyListener(new KeyAdapter() {
	...
	public void addButtonListener() {
		jbCKSH.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (pressedRow == -1) {
					JOptionPane.showMessageDialog(PersonalNewsManagerPanel.this, "请选择一条新闻！", "提示",
	...
	public void addFilterListeners() {
		jcbSHZT.addActionListener(new ActionListener() {
			@Override
		...
		jcbRQ.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				if (jcbRQ.isSelected()) {
		...
		jcbFBZT.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
		...
		jbJS.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (jcbSHZT.isSelected()) {
					...
					if ((!jcbWTJSH.isSelected()) && (!jcbTJWSH.isSelected()) && (!jcbWTGSH.isSelected())
							&& (!jcbYTGSH.isSelected()) && (!jcbFS.isSelected())) {
						sb.append("no");
					}
					sb.append("<->");
		...
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				public void run() {
					...
					// 更新表格模型
					tmPersonalNews = new PersonalNewsTableModel(PersonalNewsManagerPanel.this);
					// 初始化表格数据
					...
					// 设置表格模型
					jtPersonalNews.setModel(tmPersonalNews);
					// 更新表格外观
					initTable();
				}
			});
