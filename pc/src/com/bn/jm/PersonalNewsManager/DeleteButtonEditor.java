...
public class DeleteButtonEditor extends AbstractCellEditor implements TableCellEditor, ActionListener {// 新闻删除的JButton的编辑器
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (ztmc.equals("未提交审核") || ztmc.equals("未通过审核")) {
			int i = JOptionPane.showConfirmDialog(personalNewsPn, "确认要删除此条新闻吗?", "提示", JOptionPane.CANCEL_OPTION,
			...
			JOptionPane.showMessageDialog(personalNewsPn, "只允许删除未提交审核,未通过审核的新闻！", "提示",
	...
	public void delNEW(final String xwid)// 参数是 角色ID
	{
		...
		new Thread() {
			public void run() {
				...
				dataGeted = true;
				if (result.equals("ok")) {
					JOptionPane.showMessageDialog(personalNewsPn, "新闻删除成功！", "提示", JOptionPane.NO_OPTION);
					dataGeted = false;
					LoginWindow.watchThread();
					personalNewsPn.flushData();
					dataGeted = true;
				} else {
					JOptionPane.showMessageDialog(personalNewsPn, "新闻删除失败！", "提示", JOptionPane.WARNING_MESSAGE);
				}
			}
		}.start();
		// 监视线程
