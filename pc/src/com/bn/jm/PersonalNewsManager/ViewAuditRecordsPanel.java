...
public class ViewAuditRecordsPanel extends JPanel
{
	...
	JTable jtViewRecords = new JTable()
	{
        @Override
    ...
    //查看审核记录Table
    ViewAuditRecordsTableModel tmViewRecords;
    
    //存放表格的JScrollPane
	JScrollPane jspSHJL = new JScrollPane(jtViewRecords);
	...
	public ViewAuditRecordsPanel(final MainJFrame mf) 
	{
		this.mf=mf;
		this.setLayout(null);
		//表头设置
	...
	{
		//设置表头绘制器
		jtViewRecords.getTableHeader().setUI(new GroupableTableHeaderUI());
		//设置行高
		jtViewRecords.setRowHeight(30);
		//设置只能单选
		jtViewRecords.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		//获得table单元格绘制器
		DefaultTableCellRenderer dtcr = new DefaultTableCellRenderer();
		//设置表格里内容居中
		dtcr.setHorizontalAlignment(SwingConstants.CENTER);
		//设置第一列居中
		jtViewRecords.setDefaultRenderer(Integer.class, dtcr);
		...
		//获得表头
		JTableHeader tableHeader = jtViewRecords.getTableHeader();  
		//获得表头绘制器
		...
		//获得每一列的引用
		TableColumn tc0 = jtViewRecords.getColumnModel().getColumn(0);
		TableColumn tc1 = jtViewRecords.getColumnModel().getColumn(1);
		TableColumn tc2 = jtViewRecords.getColumnModel().getColumn(2);
		TableColumn tc3 = jtViewRecords.getColumnModel().getColumn(3);
				...
				(
				     new Runnable()
				     {
				    	 public void run()
				    	 {
				 			...
				 			//更新表格模型
				    		 tmViewRecords=new ViewAuditRecordsTableModel(ViewAuditRecordsPanel.this);
				 			//初始化表格数据
				 			...
				 			//设置表格模型
				 			jtViewRecords.setModel(tmViewRecords);	
				 			//更新表格外观
				 			initTable();
				    	 }
				     }
